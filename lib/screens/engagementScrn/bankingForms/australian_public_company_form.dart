...
class AustralianPublicCompanyForm extends StatefulWidget {
  final ScrollController? scrollController;
  const AustralianPublicCompanyForm({super.key, this.scrollController});

  @override
  State<AustralianPublicCompanyForm> createState() =>
      _AustralianPublicCompanyFormState();
}

class _AustralianPublicCompanyFormState
    extends State<AustralianPublicCompanyForm> {
  // ============ ADD FORM KEYS ============
  ...
  final List<Map<String, String>> identificationOptions = [
    ...
    {
      'value': 'stockexchange',
      'label': 'A search of the relevant domestic stock exchange',
    },
    {
      'value': 'publicdocument',
      'label': 'A public document issued by the customer',
    },
    {
      'value': 'relevantregulator',
      'label':
  ...
  // SIGNATURES LIST - UPDATED to support multiple signatures
  List<Map<String, dynamic>> signatures = [];

  // Signature Controllers for each signature
  ...
  void initState() {
    super.initState();
    // Initialize with 3 directors WITHOUT calling setState
    for (int i = 0; i < 3; i++) {
      directors.add({
        'correspondence': null,
        'title': null,
    ...
    // Initialize with 3 signatures WITHOUT calling setState
    for (int i = 0; i < 3; i++) {
      signatures.add({
  ...
  String _getFormattedDate() {
    ...
    // Returns format: "Wed Nov 12 2025"
  ...
  bool _validateUpToStep(int targetStep) {
    ...
    // Validate step 1 (Bank Detail) if trying to go to step 2 or beyond
    if (targetStep >= 2) {
    ...
    // Validate step 2 (Identification) if trying to go to step 3
    if (targetStep >= 3) {
    ...
    return Column(
      ...
      children: [
        ...
        ...directors.asMap().entries.map((entry) {
          int index = entry.key;
          return Column(children: [_buildDirectorCard(index), Spacers.sb20()]);
        }),
      ],
    );
  }
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                // Only show delete button from 4th director onwards (index >= 3)
                SizedBox(
                  width: 40.w,
                  child: index >= 3
                      ? IconButton(
        ...
        FormHelpers.buildCardWithHeader(
          ...
          Column(
            ...
            children: [
              Spacers.sb15(),
              FormHelpers.sectionTitle(
                "In respect of a domestic listed company or its majority owned subsidiary and a company licensed and subject to the regulatory oversight of a Commonwealth or State regulator please provide a copy of one identification document",
                14,
                FontWeight.w600,
                ColorsData.blackColor,
              ),
              ...
              // Wrap radio buttons with RadioGroup
              RadioGroup<String>(
    ...
    return Column(
      ...
      children: [
        ...
        ...signatures.asMap().entries.map((entry) {
          int index = entry.key;
          return Column(children: [_buildSignatureCard(index), Spacers.sb20()]);
        }),
      ],
    );
  }
  ...
  // ============ SIGNATURE CARD ============
  Widget _buildSignatureCard(int index) {
    return Card(
      elevation: 2,
      color: ColorsData.whiteColor,
      surfaceTintColor: ColorsData.whiteColor,
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          TextWidget(
                            text: "Scan to sign on your mobile device",
                            fontSize: 12,
                            fontWeight: FontWeight.w400,
                            color: Colors.grey.shade700,
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                showToast(
                                  message: "Signature retrieved from QR scan",
                                );
                              },
                              style: ElevatedButton.styleFrom(
                          ...
                          Spacers.sb8(),
                          TextWidget(
                            text: "Click Done after scanning and uploading",
                            fontSize: 10,
                            fontWeight: FontWeight.w400,
                            color: Colors.grey.shade600,
                            textAlign: TextAlign.center,
                ...
                FormHelpers.buildTextField(
                  label: "Title",
                  controller: signatures[index]['titleCntlr'],
                  hintText: "",
                  regErrorText: AppConstants.nameRegError,
                  regExp: Regx.fullNameRegExp,
                ),
                ...
                FormHelpers.buildDropdownField(
                  ...
                  value: signatures[index]['designationDropdownValue'],
                  onChanged: (value) {
                    setState(() {
                      signatures[index]['designationDropdownValue'] = value;
                    });
                  },
                ),
                Spacers.sb10(),
                FormHelpers.buildTextField(
                  label: "Name",
                  controller: signatures[index]['nameCntlr'],
                  hintText: "",
                  regErrorText: AppConstants.nameRegError,
                  regExp: Regx.fullNameRegExp,
                ),
                ...
                FormHelpers.buildTextField(
                  label: "Date",
                  controller: signatures[index]['dateCntlr'],
                  hintText: "",
                  readOnly: true,
                ),
                Spacers.sb25(),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          // Row 2: Reset / Save Draft (remains the same)
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              // Reset Button
                    ...
                    onPressed: () {
                      setState(() {
                        if (currentStep == 0) {
                          ...
                          // Clear all directors except first 3
                          while (directors.length > 3) {
                          ...
                          // Reset first 3 directors
                          for (int i = 0; i < 3 && i < directors.length; i++) {
                            directors[i]['correspondence'] = 'YES';
                            directors[i]['title'] = null;
                            (directors[i]['firstName'] as TextEditingController)
                                .clear();
                            (directors[i]['middleName']
                                    as TextEditingController)
                                .clear();
                            (directors[i]['surname'] as TextEditingController)
                                .clear();
                            (directors[i]['email'] as TextEditingController)
                                .clear();
                            (directors[i]['phone'] as TextEditingController)
                                .clear();
                          }
                        }
                      });
